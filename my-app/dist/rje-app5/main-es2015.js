...
function EditUserComponent_app_users_33_Template(rf, ctx) { if (rf & 1) {
...
class NewUserComponent {
    ...
    getAllUsers() {
        this.appService.getUsers().pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_2__["takeUntil"])(this.destroy$)).subscribe((users) => {
            this.users = users;
            _global_values__WEBPACK_IMPORTED_MODULE_4__["GlobalValues"].currentId = this.users[this.users.length - 1].id;
        });
    }
    ngOnDestroy() {
        this.destroy$.next(true);
...
class UsersComponent {
    ...
    deleteUser(user) {
        ...
        if (confirm("Are you sure you want to delete " + user.firstName + " " + user.lastName + " ?")) {
            this.appService.deleteUser(this.deletedUser.id).pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_1__["takeUntil"])(this.destroy$)).subscribe(data => {
                console.log('message::::', data);
            });
        }
        this.getAllUsers();
    ...
    getAllUsers() {
        this.appService.getUsers().pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_1__["takeUntil"])(this.destroy$)).subscribe((users) => {
            this.users = users;
            _global_values__WEBPACK_IMPORTED_MODULE_3__["GlobalValues"].currentId = this.users[this.users.length - 1].id;
            this.showList = true;
        });
    }
}
